...
这不是提示词纪律，是 `PreToolUse` 钩子在工具层拦截——并非"提醒 AI 不要跳步"，而是工具调用被 CLI 直接拒绝。

## 强度：拦得住什么、拦不住什么

以下明确其能力边界，避免过高或过低的预期。
...
因此定位为：**能拦截无意的遗忘与顺手跳步，无法阻止刻意绕行。** 兜底机制有两项：绕行操作会记入 `<项目根>/.academic_gate_audit.jsonl`，以及最终仍有人工审核。如需彻底禁止跳步，参见下方「彻底锁死」一节。
...
如需保留技能但关闭拦截，使用此开关即可，不要删除门禁目录（删除后反而会触发技能的旧安装逻辑将其重新写入 `settings.json`）。
...
**如何查看当前状态**：运行任一技能的开局脚本（`env_preflight.py` / `init_project.py`），关闭状态下会输出一段以「学术门禁的拦截层已被你关闭」开头的说明，包含开关位置、最后修改日期及用户填写的理由。状态卡**不会**显示"当前不拦"，该信息不注入模型上下文。
...
门禁自身文件已在写保护清单中，AI 通过写文件类工具无法修改。如需将命令行这一缺口也封堵，可追加以下两项措施（均须用户亲手操作，AI 无权执行）：
...
2. **用 CLI 自带的路径禁止规则**：Claude Code 的 `permissions.deny` 支持按路径禁 `Write` / `Edit` / `Bash`，把门禁目录与开关文件加进去，就不再依赖我们这层黑名单。具体写法参见 CLI 官方文档，本插件不代为修改 `settings.json`。
...
**本插件的写入范围**（仅以下三处）：`<项目根>/.academic_gate_audit.jsonl`（只记
...
上面 5 个 `.py` 与 `gate_registry.json` 跟 `_shared/` 逐字节一致，由 `sync_vendored.py` 守卫。**修改任一份均须走同步流程**，禁止单独修改此处的副本。
