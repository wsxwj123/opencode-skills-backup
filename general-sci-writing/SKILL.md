...
description: 用于撰写、润色符合Nature/Science/Cell发表标准的SCI研究论文（Article类型），适用于多学科领域的学术研究。当用户提到写论文、SCI论文、学术写作、科研写作、论文润色、研究论文、学术投稿、投稿、write SCI paper、academic writing 时优先调用。
license: Proprietary
---
...
3. **Provider 白名单**：`citation_guard.py` 允许的 provider：`pubmed-cli`（首选）、`paper-search`（CS/AI首选/备用/预印本）；`openalex-cli` 及 websearch/tavily 变体一律阻断。
...
### Phase 5: 质量控制 (`/check`)
**内容**：运行 `python scripts/state_manager.py stats` 检查各节字数；验证引用密度（Key Section ≥500词，引用≥3条）；扫描正文引用号与 `literature_index.json` 是否一致。
...
| `drug_delivery` | 药物递送系统 | 纳米药物、基因治疗等 |
...
- **论文正文严禁列点**：`Abstract/Introduction/Results/Discussion/Conclusion` 中禁用 `-`/`*`/`1.` 等列点符号；交互对话可正常使用结构化列表。Methods 配方列表例外。
- **引用格式**：正文一律 `[n]`，编号来源于分节文献矩阵重排后的全局索引，每节末附 Vancouver 格式参考文献列表。
