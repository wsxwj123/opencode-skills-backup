#!/usr/bin/env python3
"""Embedded-image extractor for .docx manuscripts (canonical, shared across
polish-sci / revise-sci / sci2doc).
...
CLI:
  python3 extract_docx_images.py --manuscript <docx> --project-root <root>
                                 [--figures-dir figures]

Exit contract:
  - Always prints a single JSON line to stdout:
      {"ok": true, "images_extracted": N, "figures_dir": "<abs path>"}
  - exit 0 on success (N >= 0).
  - exit 1 only on hard errors (manuscript missing / not a zip).

TODO: PDF extraction (PyMuPDF) is intentionally NOT implemented in this round.
"""
