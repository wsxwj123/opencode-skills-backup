...
def build_custom_ready_format_profile(
    ...
    profile = {
        ...
        "source_template_files": ["/tmp/custom_spec.pdf"],
        "requirements_summary": ["A4 版式，已按模板提取页边距和页眉页脚参数"],
        ...
        "school_code": "12345",
        "header_left_text": header_left_text or f"{university_name}{degree_type}",
    }
...
def get_east_asian_font_name(run):
...
def get_page_numbering(section):
...
class Sci2DocScriptsTests(unittest.TestCase):
    ...
    def test_state_init_command_sets_default_format_profile(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td) / "default_style_project"
            proc = run_py(
                STATE_SCRIPT,
                [
                    "--project-root",
            ...
            self.assertEqual(payload.get("format_profile", {}).get("mode"), "default_csu")
            self.assertEqual(profile.get("format_profile", {}).get("status"), "ready")
            self.assertTrue(profile.get("format_profile", {}).get("allow_docx_generation"))
            self.assertEqual(profile.get("format_profile", {}).get("university_name"), "中南大学")
            self.assertEqual(state.get("progress", {}).get("status"), "idle")

    def test_state_init_command_marks_custom_mode_as_pending_template(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td) / "custom_style_project"
            proc = run_py(
                STATE_SCRIPT,
                [
                    "--project-root",
            ...
            self.assertEqual(payload.get("format_profile", {}).get("mode"), "custom")
            self.assertEqual(profile.get("format_profile", {}).get("status"), "pending_template")
            self.assertFalse(profile.get("format_profile", {}).get("allow_docx_generation"))
            self.assertIn("页边距规范", profile.get("format_profile", {}).get("missing_requirements", []))
            self.assertEqual(state.get("progress", {}).get("status"), "pending_template")

    def test_state_init_renders_managed_front_matter_files(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td) / "front_matter_project"
            proc = run_py(
                STATE_SCRIPT,
                [
                    "--project-root",
            ...
            cover_md = root / "atomic_md" / "封面.md"
            title_md = root / "atomic_md" / "题名页.md"
            declaration_md = root / "atomic_md" / "独创性声明与授权书.md"
            abstract_md = root / "atomic_md" / "中文摘要.md"
            abstract_en_md = root / "atomic_md" / "英文摘要.md"
            toc_md = root / "atomic_md" / "目录.md"
            abbr_md = root / "atomic_md" / "缩略语表.md"
            cover_docx = root / "02_分章节文档" / "封面.docx"
            title_docx = root / "02_分章节文档" / "题名页.docx"
            declaration_docx = root / "02_分章节文档" / "独创性声明与授权书.docx"
            abstract_docx = root / "02_分章节文档" / "中文摘要.docx"
            abstract_en_docx = root / "02_分章节文档" / "英文摘要.docx"
            toc_docx = root / "02_分章节文档" / "目录.docx"
            abbr_docx = root / "02_分章节文档" / "缩略语表.docx"
            ...
            self.assertIn("managed-by-sci2doc-front-matter", cover_md.read_text(encoding="utf-8"))
            self.assertIn("中南大学博士学位论文", cover_md.read_text(encoding="utf-8"))
            self.assertIn("学校代码：10533", title_md.read_text(encoding="utf-8"))
            self.assertIn("本人授权中南大学", declaration_md.read_text(encoding="utf-8"))
            self.assertIn("摘  要", abstract_md.read_text(encoding="utf-8"))
            self.assertIn("ABSTRACT", abstract_en_md.read_text(encoding="utf-8"))
            self.assertIn("目  录", toc_md.read_text(encoding="utf-8"))
            self.assertIn("主要缩略语对照表", abbr_md.read_text(encoding="utf-8"))
            self.assertIn("暂无已注册缩略语", abbr_md.read_text(encoding="utf-8"))

    def test_render_front_matter_does_not_overwrite_unmanaged_markdown(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td) / "front_matter_protect_project"
            init_proc = run_py(
                STATE_SCRIPT,
                [
                    "--project-root",
            ...
            self.assertEqual(proc.returncode, 0, msg=proc.stdout + proc.stderr)
            self.assertEqual(cover_md.read_text(encoding="utf-8"), "# 用户自定义封面\n请勿覆盖\n")

    def test_render_front_matter_uses_project_info_and_abbreviation_registry(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td) / "front_matter_content_project"
            init_proc = run_py(
                STATE_SCRIPT,
                [
                    "--project-root",
            ...
            state.setdefault("project_info", {}).update(
                {
                    "classification": "TP391",
                    "udc": "004.8",
                    "abstract_zh": "这是中文摘要正文，用于验证自动渲染。",
                    "keywords_zh": ["人工智能", "医学图像"],
                    "abstract_en": "This is the English abstract body.",
                    "keywords_en": ["artificial intelligence", "medical imaging"],
                }
            )
            state_path.write_text(json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8")
            (root / "abbreviation_registry.json").write_text(
                json.dumps(
                    {
                        "AI": {
                            "full_cn": "人工智能",
                            "full_en": "Artificial Intelligence",
                            "first_chapter": "1",
                            "first_section": "1.1",
                        }
                    },
            ...
            zh_abstract = (root / "atomic_md" / "中文摘要.md").read_text(encoding="utf-8")
            en_abstract = (root / "atomic_md" / "英文摘要.md").read_text(encoding="utf-8")
            abbr_md = (root / "atomic_md" / "缩略语表.md").read_text(encoding="utf-8")
            self.assertIn("这是中文摘要正文，用于验证自动渲染。", zh_abstract)
            self.assertIn("人工智能；医学图像", zh_abstract)
            self.assertIn("This is the English abstract body.", en_abstract)
            self.assertIn("artificial intelligence; medical imaging", en_abstract)
            self.assertIn("| AI | Artificial Intelligence | 人工智能 |", abbr_md)
    ...
    def test_markdown_to_docx_blocks_custom_pending_template_projects(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / "thesis_profile.json").write_text(
                json.dumps(
            ...
            md = root / "03_合并文档_md" / "完整博士论文.md"
            md.parent.mkdir(parents=True, exist_ok=True)
            out = root / "03_合并文档" / "完整博士论文.docx"
            md.write_text("# 第一章 绪论\n\n正文内容\n", encoding="utf-8")

            proc = run_py(MD_SCRIPT, [str(md), "-o", str(out), "--project-root", str(root)])
            self.assertNotEqual(proc.returncode, 0)
            self.assertIn("pending_template", proc.stdout + proc.stderr)

    def test_atomic_md_merge_full_includes_front_matter_from_atomic_root(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            init_proc = run_py(
                STATE_SCRIPT,
            ...
            md_dir = root / "02_分章节文档_md"
            md_dir.mkdir(parents=True, exist_ok=True)
            (md_dir / "第1章_绪论_合并.md").write_text("# 第一章 绪论\n\n正文内容\n", encoding="utf-8")
            ...
            self.assertEqual(proc.returncode, 0, msg=proc.stdout + proc.stderr)
            payload = json.loads(proc.stdout)
            merged_text = Path(payload["output_md"]).read_text(encoding="utf-8")
            self.assertIn("中南大学博士学位论文", merged_text)
            self.assertIn("前置部分合并测试", merged_text)
            self.assertIn("# 第一章 绪论", merged_text)

    def test_markdown_to_docx_applies_ready_custom_format_profile(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            format_profile = build_custom_ready_format_profile(
                university_name="示例理工大学",
                degree_type="硕士学位论文",
            )
            (root / "thesis_profile.json").write_text(
                json.dumps({"format_profile": format_profile}, ensure_ascii=False, indent=2),
                encoding="utf-8",
            )
            md = root / "03_合并文档_md" / "完整博士论文.md"
            md.parent.mkdir(parents=True, exist_ok=True)
            ...
            self.assertAlmostEqual(section.top_margin.cm, 2.8, places=2)
            self.assertAlmostEqual(section.bottom_margin.cm, 2.6, places=2)
            self.assertAlmostEqual(section.left_margin.cm, 3.0, places=2)
            self.assertAlmostEqual(section.right_margin.cm, 3.1, places=2)
            self.assertAlmostEqual(section.header_distance.cm, 1.2, places=2)
            self.assertAlmostEqual(section.footer_distance.cm, 1.6, places=2)
            self.assertIn("示例理工大学硕士学位论文", section.header.paragraphs[0].text)
            self.assertIn("第一章 绪论", section.header.paragraphs[0].text)

    def test_markdown_to_docx_applies_dynamic_typography_profile(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            format_profile = build_custom_ready_format_profile(
                university_name="示例理工大学",
            ...
            figure_caption = next(p for p in doc.paragraphs if p.text.strip() == "图 1-1：示意图")
            table_caption = next(p for p in doc.paragraphs if p.text.strip() == "表 1-1：结果")

            self.assertAlmostEqual(h1.runs[0].font.size.pt, 18.0, places=1)
            self.assertEqual(get_east_asian_font_name(h1.runs[0]), "FangSong")
            self.assertEqual(h1.paragraph_format.alignment, 0)
            self.assertAlmostEqual(body.runs[0].font.size.pt, 11.0, places=1)
            self.assertEqual(get_east_asian_font_name(body.runs[0]), "KaiTi")
            self.assertAlmostEqual(body.paragraph_format.first_line_indent.cm, 0.5, places=2)
            self.assertAlmostEqual(figure_caption.runs[0].font.size.pt, 11.0, places=1)
            self.assertEqual(get_east_asian_font_name(figure_caption.runs[0]), "SimSun")
            self.assertAlmostEqual(table_caption.runs[0].font.size.pt, 11.0, places=1)
            self.assertEqual(get_east_asian_font_name(table_caption.runs[0]), "SimHei")
            self.assertEqual(doc.tables[0].rows[0].cells[0].paragraphs[0].paragraph_format.alignment, 0)
            header_run = doc.tables[0].rows[0].cells[0].paragraphs[0].runs[0]
            self.assertFalse(header_run.font.bold)
            self.assertEqual(get_east_asian_font_name(header_run), "FangSong")

    def test_check_quality_respects_dynamic_typography_profile(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            format_profile = build_custom_ready_format_profile(
                university_name="江海大学",
                degree_type="硕士学位论文",
                style_profile={
                    "heading1": {
                        "font_size_pt": 18,
            ...
            md_dir = root / "03_合并文档_md"
            md_dir.mkdir(parents=True, exist_ok=True)
            md_path = md_dir / "完整博士论文.md"
            md_path.write_text("# 第一章 绪论\n\n这是正文段落内容。\n", encoding="utf-8")
            docx_path = root / "03_合并文档" / "完整博士论文.docx"

            md_proc = run_py(MD_SCRIPT, [str(md_path), "-o", str(docx_path), "--project-root", str(root)])
            self.assertEqual(md_proc.returncode, 0, msg=md_proc.stdout + md_proc.stderr)

            proc = run_py(QUALITY_SCRIPT, [str(docx_path), "--output", "json"])
            self.assertIn(proc.returncode, (0, 1), msg=proc.stdout + proc.stderr)
            payload = json.loads(proc.stdout)
            issue_text = json.dumps(payload.get("issues", []), ensure_ascii=False)
            self.assertNotIn("一级标题字号不符", issue_text)
            self.assertNotIn("一级标题中文字体不符", issue_text)
            self.assertNotIn("正文字号不符", issue_text)
            self.assertNotIn("正文中文字体不符", issue_text)
    ...
    def test_merge_documents_add_header_uses_custom_format_profile(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            ...
            for name, text in [
                ...
                d = Document()
                d.add_heading(name.replace(".docx", ""), level=1)
                d.add_paragraph(text)
            ...
            self.assertIn("海河大学博士学位论文", header_text)
    ...
    def test_merge_documents_compat_wrapper_uses_current_layout(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            merged_text = "\n".join(p.text for p in merged_doc.paragraphs if p.text.strip())
            self.assertIn("默认布局测试论文", merged_text)
    ...
    def test_check_quality_blocks_custom_pending_template_projects(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / "thesis_profile.json").write_text(
                json.dumps(
            ...
            self.assertEqual(payload.get("error"), "pending_template")
            self.assertIn("页眉页脚要求", json.dumps(payload, ensure_ascii=False))

    def test_check_quality_uses_ready_custom_format_profile_for_layout_checks(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            format_profile = build_custom_ready_format_profile(
                university_name="江海大学",
            ...
            payload = json.loads(proc.stdout)
            issue_text = json.dumps(payload.get("issues", []), ensure_ascii=False)
            self.assertNotIn("页眉缺少", issue_text)
            self.assertNotIn("页眉距顶端", issue_text)
            self.assertNotIn("上边距不符", issue_text)
    ...
    def test_state_profile_command_updates_custom_format_profile(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            init_proc = run_py(
                STATE_SCRIPT,
                ["--project-root", str(root), "init", "--title", "自定义配置测试"],
            )
            self.assertEqual(init_proc.returncode, 0, msg=init_proc.stdout + init_proc.stderr)
            ...
            self.assertEqual(payload.get("profile", {}).get("format_profile", {}).get("mode"), "custom")
            self.assertEqual(profile.get("format_profile", {}).get("status"), "ready")
            self.assertEqual(profile.get("format_profile", {}).get("university_name"), "示例大学")
            self.assertIn("/tmp/custom_spec.pdf", profile.get("format_profile", {}).get("source_template_files", []))
            self.assertEqual(profile.get("format_profile", {}).get("page_margins_cm", {}).get("top"), 2.8)

            cover_text = (root / "atomic_md" / "封面.md").read_text(encoding="utf-8")
            title_text = (root / "atomic_md" / "题名页.md").read_text(encoding="utf-8")
            declaration_text = (root / "atomic_md" / "独创性声明与授权书.md").read_text(encoding="utf-8")
            self.assertIn("示例大学博士学位论文", cover_text)
            self.assertIn("学校代码：12345", title_text)
            self.assertIn("本人授权示例大学", declaration_text)

    def test_state_profile_command_accepts_format_profile_json(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            init_proc = run_py(
                STATE_SCRIPT,
                ["--project-root", str(root), "init", "--title", "JSON 格式配置测试"],
            )
            self.assertEqual(init_proc.returncode, 0, msg=init_proc.stdout + init_proc.stderr)
            ...
            profile = json.loads((root / "thesis_profile.json").read_text(encoding="utf-8"))
            style_profile = profile.get("format_profile", {}).get("style_profile", {})
            self.assertEqual(style_profile.get("heading1", {}).get("font_size_pt"), 18.0)
            self.assertEqual(style_profile.get("heading1", {}).get("font_east_asia"), "FangSong")
            self.assertEqual(style_profile.get("body", {}).get("font_size_pt"), 11.0)
            self.assertEqual(style_profile.get("body", {}).get("font_east_asia"), "KaiTi")

    def test_state_profile_command_rejects_invalid_format_profile_json(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            init_proc = run_py(
                STATE_SCRIPT,
                ["--project-root", str(root), "init", "--title", "非法格式 JSON 测试"],
            )
            self.assertEqual(init_proc.returncode, 0, msg=init_proc.stdout + init_proc.stderr)
            ...
            self.assertNotEqual(proc.returncode, 0)
            payload = json.loads(proc.stdout)
            self.assertEqual(payload.get("error"), "invalid_json_payload")
            self.assertIn("format_profile.unexpected", payload.get("detail", ""))

    def test_state_profile_command_accepts_project_info_json(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            init_proc = run_py(
                STATE_SCRIPT,
                ["--project-root", str(root), "init", "--title", "JSON 前置内容测试"],
            )
            self.assertEqual(init_proc.returncode, 0, msg=init_proc.stdout + init_proc.stderr)
            ...
            proc = run_py(
                ...
                [
                    ...
                    json.dumps(
                        {
                            "abstract_zh": "这是通过 project-info-json 写入的中文摘要。",
                            "keywords_zh": ["肿瘤学", "人工智能"],
                            "abstract_en": "This abstract comes from project-info-json.",
            ...
            self.assertEqual(project_info.get("classification"), "R73")
            self.assertEqual(project_info.get("abstract_zh"), "这是通过 project-info-json 写入的中文摘要。")

            zh_abstract = (root / "atomic_md" / "中文摘要.md").read_text(encoding="utf-8")
            en_abstract = (root / "atomic_md" / "英文摘要.md").read_text(encoding="utf-8")
            self.assertIn("这是通过 project-info-json 写入的中文摘要。", zh_abstract)
            self.assertIn("肿瘤学；人工智能", zh_abstract)
            self.assertIn("This abstract comes from project-info-json.", en_abstract)
            self.assertIn("oncology; artificial intelligence", en_abstract)

    def test_state_profile_command_rejects_invalid_project_info_json(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            init_proc = run_py(
                STATE_SCRIPT,
                ["--project-root", str(root), "init", "--title", "非法项目信息 JSON 测试"],
            )
            self.assertEqual(init_proc.returncode, 0, msg=init_proc.stdout + init_proc.stderr)
            ...
            payload = json.loads(proc.stdout)
            self.assertEqual(payload.get("error"), "invalid_json_payload")
            self.assertIn("project_info.keywords_zh", payload.get("detail", ""))

    def test_state_profile_command_accepts_page_numbering_json_and_renders_front_matter_docx(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            init_proc = run_py(
                STATE_SCRIPT,
                ["--project-root", str(root), "init", "--title", "页码配置测试"],
            )
            self.assertEqual(init_proc.returncode, 0, msg=init_proc.stdout + init_proc.stderr)
            ...
            profile = json.loads((root / "thesis_profile.json").read_text(encoding="utf-8"))
            page_numbering = profile.get("format_profile", {}).get("page_numbering", {})
            self.assertEqual(page_numbering.get("front_matter", {}).get("format"), "upperRoman")
            self.assertEqual(page_numbering.get("front_matter", {}).get("start"), 3)

            zh_abstract_docx = root / "02_分章节文档" / "中文摘要.docx"
            self.assertTrue(zh_abstract_docx.exists())
            doc = Document(str(zh_abstract_docx))
            self.assertEqual(get_page_numbering(doc.sections[0]), {"fmt": "upperRoman", "start": "3"})

    def test_state_profile_command_can_resolve_pending_template_to_ready(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            init_proc = run_py(
                STATE_SCRIPT,
            ...
            self.assertEqual(profile.get("format_profile", {}).get("status"), "ready")
            self.assertEqual(profile.get("format_profile", {}).get("missing_requirements"), [])
            self.assertTrue(profile.get("format_profile", {}).get("allow_docx_generation"))
            self.assertEqual(state.get("progress", {}).get("status"), "idle")

    def test_state_profile_command_keeps_custom_mode_pending_without_layout_numbers(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            init_proc = run_py(
                STATE_SCRIPT,
                ["--project-root", str(root), "init", "--title", "缺少结构化版式参数"],
            )
            self.assertEqual(init_proc.returncode, 0, msg=init_proc.stdout + init_proc.stderr)
            ...
            self.assertEqual(profile.get("format_profile", {}).get("status"), "pending_template")
            self.assertIn("页边距规范", profile.get("format_profile", {}).get("missing_requirements", []))
            self.assertIn("页眉页脚距离规范", profile.get("format_profile", {}).get("missing_requirements", []))
