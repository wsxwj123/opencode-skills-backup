...
def bootstrap(repo_url, branch, force_origin=False, proxy_port=None):
    ...
    ports = resolve_proxy_ports(proxy_port)
    pushed = None
    used_port = None
    for port in ports:
        set_proxy_env(port)
        ...
        if pushed is not None:
            ...
            break
    if pushed is None:
        print("⚠️ Push failed on all proxy ports. Tried:", ", ".join(ports))
        print("Verify repo URL, proxy port, and GitHub credentials, then retry.")
        return 1
    print(f"✅ Push succeeded via proxy port: {used_port}")
...
if __name__ == "__main__":
    ...
    raise SystemExit(bootstrap(args.repo_url, args.branch, args.force_origin, args.proxy_port))
