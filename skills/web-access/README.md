...
给 Claude 或 OpenCode 装上完整联网能力的 skill。

Claude 或 OpenCode 原本有 WebSearch、WebFetch，但缺少调度策略和浏览器自动化能力。这个 skill 补上的是：**联网策略 + CDP 浏览器操作 + 站点经验积累**。
...
# OpenCode（已安装到全局 skills 时）
git clone https://github.com/eze-is/web-access ~/.config/opencode/skills/web-access
```
...
# 启动（OpenCode）
node ~/.config/opencode/skills/web-access/scripts/cdp-proxy.mjs &
