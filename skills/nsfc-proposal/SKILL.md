...
## Tooling Rules
Academic literature retrieval must follow this priority order:

1. **PubMed CLI (Primary):** Use `esearch`/`efetch`/`einfo` (path `~/edirect/`). Must append `< /dev/null`, use proxy `http_proxy=http://127.0.0.1:7897`.
   Example: `export http_proxy=http://127.0.0.1:7897 && esearch -db pubmed -query "xxx" < /dev/null | efetch -format abstract`
2. **OpenAlex CLI (Cross-domain supplement):** Use `pyalex` (python3).
   Example: `from pyalex import Works; Works().search("xxx").get()`
3. **paper-search MCP (Fallback / Preprints only):** Use when CLI tools fail, or for arXiv/bioRxiv/medRxiv preprints.
   MCP tool names: `mcp__paper-search__search_pubmed`, `mcp__paper-search__search_biorxiv`, `mcp__paper-search__search_medrxiv`

Do not use generic web search/fetch tools for citation evidence in proposal claims.
**严禁** 使用 `tavily` 或 `websearch`，无论有无 DOI/PMID.
...
4. Phase 3: write 研究方案/技术路线（P3）.
   ...
   **Figure Prompt Generation（技术路线图与框架图的AI绘图提示词）：**
   For each figure required in the proposal (technical roadmap, research framework, preliminary data schematics), generate a Figure Prompt block and save to `sections/figure_prompts.md`:
   ...
   Generation rules:
   - Technical Roadmap figure is MANDATORY in Phase 3 (研究方案); generate its prompt during Phase 3 writing
   - Research Framework figure (总体框架) is recommended in Phase 1 (立项依据); generate if the research logic benefits from visual representation
   - For preliminary data figures: describe the expected figure type (Western blot, immunofluorescence, etc.) with placeholder notation [Preliminary Data Fig N]
   - All proposal figures must use a consistent color palette (dark blue = main narrative, green = innovations, orange = expected deliverables)
   - Figure prompts must align with the H/O/RC/KSQ consistency_map.json — each figure should visually represent at least one RC (Research Content)
