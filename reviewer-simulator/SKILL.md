...
3. **Claude Code 调用方式**：用 `TaskCreate` 工具（或等效的 spawn_task）为每个视角创建独立任务，模型默认 `claude-sonnet-4-6`（除非用户指定），任务提示中包含视角 rubric 与稿件内容，task 之间无上下文共享。也可直接用 `academic-blind-reviewer` 预定义subagent（若已配置）。
...
2. **派一个独立subagent**（Claude Code 用 `academic-blind-reviewer`；其他平台派通用subagent，默认 sonnet 模型），把任务包原样给它、**不要给它本次审稿的分析上下文**，要求按任务包返回 JSON 数组。
