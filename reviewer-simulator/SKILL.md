...
**环境预检（软门禁，初始化 data/ 前）：** `python scripts/env_preflight.py $WORKROOT --cli esearch`，写 `env_status.json`，末行 `PRECHECK: OK|ASK|BLOCKED`。`BLOCKED`（Python 过低）→ 停并引导升级；`ASK`（缺 esearch 等可选工具）→ 逐项问用户是否安装并给指引，用户答"已装/不装"后才继续；`OK` → 继续。git 仅信息记录（本技能不产正文、不建 git 检查点）。
