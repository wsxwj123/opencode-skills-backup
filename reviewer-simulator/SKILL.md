...
2. **数量参考指标**：致命/大修问题建议10条左右；小修/格式意见建议10条左右；**均以实际问题为准（见第六部分第5条，禁止凑数）**。18点深度分析中，核心点详写（≥150字），次要点精简（≥80字）。
...
列出次要但需修改的问题，建议10条左右，以实际问题为准（见第六部分第5条）
每条必须按照统一格式呈现:
...
从第七部分"必须解决的核心问题"中选择3至5个最致命的问题，逐一展开：
...
3. 每条观点都应给出稿件内的证据锚点；如稿件缺乏证据，明确写出证据缺失。
