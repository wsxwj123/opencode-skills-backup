...
- 三返修技能关系：reviewer-response(只出回复信)/revise(改稿+回复信)/polish(只润色语言)管不同场景；真冗余是 revise 重复实现了回复信，建议复用 reviewer-response 逻辑去重，不合并成巨技能。
- 子代理不能再派子代理(Claude Code 设计：子代理无 Task 工具，必须主会话扁平派)；所以盲检必须主会话派。

### 遗留(非阻断)
- mimo-v2.5-pro 模型配置(用户定)
- review Phase2 "Round 1"命名小修(可选)
- revise 复用 reviewer-response 回复信逻辑去重(可选)
- nsfc Phase 0.5 真稿实测
