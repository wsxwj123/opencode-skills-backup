...
4. **Compile:** Merge all section drafts in correct order:
   ...
   - **① 合成语料（本步开头无条件覆盖重建，幂等）**：综述的图题注登记在 `figures/figure_index.md`、按设计**不在成稿里**（Phase 4 Step 4 的 `cat` 编译一个字不改，题注不塞进成稿），直接把成稿喂第 1 层 = 每张注册过的图都被判悬空 = **100% 系统性假阳**（再被第 3 层"找不到定义处→confirmed"全部坐实）。故先把注册题注行拼到成稿前面：
     ...
     > **该修正的已知副作用（无害，但别当成 bug 查）**：全角注册行既然不被题注正则识别，就**不会进 `caption_rows`**，于是会被当成一条标题、多抽出一条 `number=null` 的**假 section 锚**（实测 `SEC [(None,'Figure 0：概念框架图'), ('2.1','Results')]`）。这使「喂题注前后 `sections` 逐条相同」这条不变式**只在全部注册行都用半角分隔符时成立**。假 section 无编号、也不带 `#` 编号标题形态，节引用回查按④ 护栏 3 的判据不会命中它 → **不产假阳**，只是 `outline.json` 多一条噪声。
   ...
   - **③ 锚可用性门（第 2 层开跑前的确定性前置判定，主 agent 算好写进 prompt）**：
     ...
     **为什么禁 `uncertain` 而不是"标 uncertain 走人工"**：`uncertain` 与 `missing_target` 共用第 3 层同一条极性模板（"连定义处都找不到 → pass=confirmed"），锚不可用时目标本来就没有定义处 → 每条 uncertain 都会被自动坐实成 confirmed → 假阳 HALT。这与补充材料 S 前缀是同一个形状的坑，处置必须一致。
     - **🔴 类级 `any(...)` 只是 advisory 的下限触发，不是"这一类要不要查"的开关**：锚集合为空 → 该类整类 skip + advisory（上一段，照旧）；但**锚集合非空 ≠ 可以直接下判** —— 真正的把关在 ④ 护栏 3 的**逐条定义形态回查**，每条 `missing_target` 都必须先过那一关才允许报。
     - **🔴 `sections_anchor_available` 这个算法本身不可信（实测）**：第 1 层分不清"编号标题"和"正文里的普通编号列表"，一份标题全无编号的稿只要正文写了 `1. 代谢物通路` / `2. 免疫通路`，抽出来就是 `SEC [(None,'综述标题'),(None,'背景'),('1','代谢物通路'),('2','免疫通路')]` → `any(...)` 被翻成 `true`、类级门根本不触发 → `如前文 2.1 节所述` 一路走到第 3 层被"找不到定义处→pass"坐实成**假阳 HALT**。而本技能**不强制**标题带节号、综述正文**必有编号列举**，这不是边角场景。**所以节类是否可核，改用 ④ 护栏 3 的"节定义形态"重算**：拿那条判据扫一遍 `tmp/xref_corpus.md`，**一行都不存在**符合节定义形态的行 → 节锚判定为不可用 → **节类整类强制 skip + advisory**（`any(...)` 说 true 也不算数）。
     - **三类的实情**：**图**——题注在成稿外，靠 ① 合并语料才查得了；Polish 模式登记表为空 → 整类 skip。**表**——**一等公民**，题注就写在正文里、直接抽（systematic 档强制四类表：exclusions-with-reasons / RoB / effect / SoF，量大且核心）；作者一张表题注都没写才 skip。**节**——按上一条重算：稿里存在真小节标题形态才核（命中即无问题、未命中且过了逐条回查才报 `missing_target`），全篇只有编号列举 → 整类 skip。
   ...
   - **④ 第 2 层 独立检测子代理（fresh context、非作者自评）**：派一个**没参与撰写**的独立子代理（`TaskCreate`/spawn_task），**只喂** `./outline.json`（去掉 `items` 字段）+ `tmp/xref_corpus.md`，**不给撰写过程上下文、不给 `outline.md` 提纲、不给作者意图**（防继承作者确认偏误 → 系统性假阴）。逐条指向型表述对着 `outline.json`（**唯一权威真值**，不得凭记忆假设稿子有某小节）判**存在性**（目标编号在不在 → `missing_target`）与**语义对应**（编号在但指错内容 → `semantic_mismatch`），拿不准标 `uncertain` 不硬判。产出 schema `[{"ref_id","citing_location","cited_target","issue_type":"missing_target|semantic_mismatch|uncertain","evidence_quote","outline_says","finding","severity"}]`（`severity` 仅信息字段，**HALT 不按它路由**，所有 confirmed 一律交作者裁决）。三道护栏逐条写进 prompt：
     ...
     - **🔴 护栏 3 · 定义形态逐条回查（三类通用，防第 1 层漏抽造成的假悬空）**：判**任何** `missing_target` 前——**图、表、节三类一视同仁，不分类别**——必须先到 `tmp/xref_corpus.md` 里回查该编号的**定义形态**，**命中任一即不报**（改判无问题）；只有连一处定义形态都找不到，才允许判该编号悬空。**判据要泛化、不是逐例枚举。**
       - **🔴 节类的适用范围更宽：任何节类 finding 都要先过本回查，含 `semantic_mismatch`**（不只 `missing_target`）。因为 `outline.json` 的节锚本身会被正文编号列表污染（正文写 `1. 代谢物通路` 就会多出一条 `('1','代谢物通路')` 的假节锚），拿这种假锚当真值去判"编号在、但指的内容对不上"同样会假阳。节类一律以**本回查的结果**为准，`outline["sections"]` 只作参考。
       ...
       - **节的定义形态（小节标题行）**：该行**必须带 markdown 标题标记 `#`～`######`（或 docx 标题样式）**；去掉标题标记与空白后**以被查编号本身开头**（查 `2.1` 就得是 `2.1` 打头，不把 `2.1.3`、`2.10` 或句中的 `2.1` 算进来），编号后接空白或半/全角标点，其后是该节的**标题文字**。**只认带标题标记的行，裸编号行一律不认**——理由见下条。
       - **🔴 为什么收紧到"必须带 `#`"（本轮的减法，别再放开）**：上一版还认"独占一行、不以句末标点结尾、明显短于正文段落"的裸编号行，实测**这条判据精确描述了世界上每一个列表项**——`1、代谢物通路`（中文顿号不加空格，中文列举的压倒性写法）、孤立单条 `2 免疫通路证据`、正文多级列举 `2.1 …`/`2.2 …` 全部命中，节类闸门被打开后稿里其他节引用**连带**被坐实成假阳。再靠"列表项 carve-out"反向排除是**在正判据上打补丁**（中文标点、单条列举、多级列举各漏一个），与本轮砍条目类时认定的"在正交判据上砌墙"是同一种错误，故 carve-out **整段删除**、改用减法。**成稿是 `cat drafts/section_*.md` 编译出来的 markdown、各节由本技能自己的撰写子代理写，"不带 `#` 的裸编号标题"在这条产线上不是真实场景**；为兜这个不存在的场景而放开判据，漏进来的是每一个列表项。收紧后失败方向统一为**漏报**（真写了裸编号标题的稿 → 节类整类 skip + advisory 交人工），**不产假阳**。
