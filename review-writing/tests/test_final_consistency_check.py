...
class FinalConsistencyCheckTests(unittest.TestCase):
    ...
    def test_ignores_placeholder_sections(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "drafts").mkdir()
            (root / "data").mkdir()
    ...
    def test_checks_deeper_storyline_headings(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "drafts").mkdir()
            (root / "data").mkdir()
            ...
            (root / "data" / "literature_index.json").write_text(json.dumps([]), encoding="utf-8")
            (root / "data" / "synthesis_matrix.json").write_text(json.dumps([]), encoding="utf-8")
