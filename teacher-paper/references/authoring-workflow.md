...
{"type":"material","label":"【材料一】","title":"天气预报中的统计与概率",
 "paras":["……正文（楷体首行缩进）……"],"source":"（材料均节选自《科学之友》）"}

// 小说/散文：标题+作者+正文（1000-1500字）+出处；block 可选，显式声明字数门禁板块归属
{"type":"material","title":"老街","author":"佚名","block":"小说",
...
### 选文排版铁律速查（语文 + 英语阅读/完形必守，build 有硬门禁）

| # | 规则 | 落地方式 |
|---|------|----------|
| 1 | **选文必须随卷给全文**——否则学生无法作答 | `material` 块且 `paras` 写全文；缺则 build **拒绝出卷** |
| 2 | **非连续性文本**选文末尾**标注出处**，右对齐 | `material.source` 填 `（材料节选自《科学之友》）` 等，脚本自动右对齐 |
| 3 | **小说/文言/古诗/名著**须给**标题 + 作者/朝代或出处** | `material.title` + `material.author` + `material.source` |
| 4 | **选文字数符合板块区间**（如九年级小说 1000-1500 净字数，全表见对应科目文档第 2 节，init 写入 `meta.block_len`）| 越界 build **拒绝出卷**（`--allow-length` 可降级为告警）|
| 5 | **古诗词整体居中**（标题/作者/诗句），不要合并成一段 | `material.layout="verse"`，脚本逐句居中 |
| 6 | 选文正文默认**楷体**、首行缩进2字；标题/作者楷体居中；出处宋体右对齐 | make_paper.py 自动处理，无需手动设字体 |
...
🔴 **忠实节选铁律（build 有 `_check_faithful_excerpt` 硬门禁）**：选文**可删掉与命题无关的部分（含中间段落），但保留的每段文字必须逐字取自原文**——禁改字、禁编造、禁打乱原文顺序。material 每段 `para` 去标点空白后须为 `source_file` 原文的逐字子串、且按原文先后顺序，否则 build **拒绝出卷**（exit 2）。卷面 `source` 一律 **"（节选自《X》）"**，禁用"改编自/整理自/原创"。原文有合理格式差异（OCR/换行噪声）误伤时才加 `--allow-excerpt`。
