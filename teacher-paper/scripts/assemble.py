...
def _check_material_wording(paper, warn):
    """选文标注措辞校验（软提醒）：material 的标注性字段（source/title/label/author）
    出现'改编/改写/编译/整理自…'等暗示二次创作的措辞时提醒改'节选自'；出现'原创'
    字样时提醒卷面不应自称原创（原创声明只写在 meta.source='原创-已声明'）。
    真实性铁律要求忠实节选——逐字、可删减，禁重排/改写/编造。
    ...
    for b in paper:
        if isinstance(b, dict) and b.get("type") == "material":
            for field in ("source", "title", "label", "author"):
                ...
                if not val:
                    continue
                hit = [w for w in _WORDING_BAD if w in val]
                if hit:
                    warn.append(f"选文{field}『{val}』含{'/'.join(hit)}——按真实性铁律须忠实节选"
                                f"（逐字可删减、禁重排改写编造），标注请改为'节选自…'。")
                if "原创" in val:
                    warn.append(f"选文{field}『{val}』含'原创'字样——卷面不应自称原创，"
