...
- **git（强烈建议）**：`setup.py` 会扫描 git。有 git 时 `assemble.py init` 自动把工程纳入版本管理，每完成一题 `assemble.py commit` 存档、出错可回滚；无 git 自动跳过（出卷功能不受影响，但失去逐题回滚保护，建议提示用户安装）。
...
### Phase 3.9：独立子代理盲检（v3.26.0 新增·防自评偏差）

**为什么**：Phase 3.5/3.8 是**机器规则**（题号对账/字数/sha256），查不了"内容对不对"。而出题的主会话 AI 自己复核，有"自己出的题自己觉得没问题"的乐观偏差（darwin-skill 实证：同上下文自评不可信）。所以机器两表全绿后，**必须派独立子代理盲检**——机器管结构、子代理管内容、且独立于出题过程。

🔴 **强制协议**（不可主会话自己审了事）：

1. 用 **Agent 工具派一个 sonnet 子代理**（全新上下文，**不告诉它出题过程/不复用主会话**），只给它：工程路径、`build/` 两份 Word（或 content.md）、`Phase3.5_自审表.md`、`终审表.md`。
2. 子代理任务＝**站在审卷老师立场盲审**，复核机器查不出的内容层（每条给出题号 + 问题 + 修改建议）：
...
3. 子代理回报问题清单 → 主 AI **只改对应 `items/NN_qXX.json`** → `commit` 存档 → 重 build → **重新派盲检**，直到子代理无实质异议。

> 子代理是"独立第二双眼睛"，不是橡皮图章。它报的问题主 AI 不得擅自忽略，须逐条处理或向用户说明为何不改。

🔴 **STOP·盲检出口**：独立子代理确认无实质问题后，才进入 Phase 4。
...
3. **触发的告警**：选文字数越界、题量分值不符、抓取换源、配图降级占位、是否用了任何 `--allow-*` 开关（`--allow-unsourced`/`--allow-missing-figure`/`--allow-length`/`--allow-incomplete`/`--allow-wording`/`--allow-excerpt`）——如实转达。
