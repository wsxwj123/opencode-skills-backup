...
> 🔴 **科目=英语时，必须在本轮就确认听力处理方式（三选一：省略/文字稿/用户自备音频，不得跳过）**，写入 `meta.decisions.listening_plan`——三个选项的完整含义与执行规则见 `references/subjects/英语.md` 第 5 节（唯一锚点）。**不得在写到听力大题时才发现无法生成音频**。
