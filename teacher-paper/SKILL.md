...
> 这些参数对应 `assemble.py init` 的 `--stage --subject --region --type --total --duration --questions`，以及结构 JSON（`--blueprint-file`）。**理科须在本轮确认配图方案（三选一，选项见"适用范围⚠️"）**，将选择写入 `meta.decisions.figure_plan`，Phase 2/Phase 3 严格按此执行。
...
> ⚠️ **高中政史地有预设但须核对**：`presets/` 已有 `高三_历史.json`、`高三_地理.json`、`高三_思想政治.json`，可命中使用；但各省考纲差异大，预设为通用近似值。接到高中政史地出卷请求时，**必须在 Phase 1 第一轮告知用户**："内置预设基于通用高考骨架，建议提供一份本地样卷让我解析，否则分值/大题结构可能与你校/本省实际不符。"用户无样卷→用预设兜底，并在交付免责清单中注明。
