---
name: teacher-paper
description: "教师智能出题技能 - 覆盖小学一年级到高三全科目（语文、英语、政治/道德与法治、历史、地理、数学、物理、化学、生物），全题型，按用户提供的材料/真题样卷/题型分值分布参数化出卷；结合本地资料（Word/PDF/PPT/Excel/MD/TXT/图片）与在线真实素材（新闻/科普/公版古籍/英语分级读物等）生成学生试卷，产出可直接打印的 Word 试卷 + Word 参考答案及解析。默认九年级语文·长沙中考。理科含图题须用户提供图片，本技能不自动配图。Use when: 用户要求出题、组卷、制作试卷、生成练习题、制作考试卷，或提到'出题''组卷''试卷''考试卷''练习题'等关键词。"
user-invocable: true
...
> ⚠️ **理科（数学/物理/化学/生物）配图限制**：理科题目常含几何图、电路图、坐标图、实验装置图等，本技能**不自动生成图形**。处理方式：含图题在题干用文字描述代替（如"如图所示，…"→改为详细文字叙述），或在 `figure` 块通过 `src` 插入用户提供的图片；build 会在缺图位置输出 `［图：…］` 占位，需人工补图后交付。
...
2. **内置预设**：`presets/` 下已备 13 个预设文件，按 `学段_科目[_地区].json` 命中即用；科目别名（政治/道法→思想政治）与地区缺省自动回退基名。

   | 学段 | 科目 | 文件名 |
   |------|------|--------|
   | 九年级 | 思想政治 / 历史 / 地理 / 英语 / 数学 / 物理 / 化学 / 生物 | `九年级_思想政治.json` … `九年级_生物.json` |
   | 高三 | 语文 / 英语 / 历史 / 地理 / 思想政治 / 数学 / 物理 / 化学 / 生物 | `高三_语文.json` … `高三_生物.json` |
...
> **Phase 0 → Phase 1 依赖**：Phase 0 读完资料后，样卷结构 JSON 和 materials/ 文件才就绪，Phase 1 第三轮（样卷解析）和第四轮（素材逐块确认）才能做完整确认。无资料时可直接进 Phase 1，样卷/素材在 Phase 2 出题时再补。
...
**第一轮·定位**：①学段年级（**默认九年级**；小学1-6 / 初中7-9 / 高一~高三）②科目（**默认语文**；全科支持：语文/英语/政治·道法/历史/地理/数学/物理/化学/生物。**遇数学/物理/化学/生物等理科 → 必须告知配图限制：本技能不自动生成图形，含图题须改文字描述或由用户提供图片；确认后继续**）③考试类型（**默认中考模拟**；单元/月考/期中/期末/学考/高考模拟）

> 🔴 **科目=英语时，必须在本轮就确认听力处理方式（三选一，不得跳过）**：①省略听力（总分相应下调或分值并入其它板块）；②随卷附听力文字稿（把听力材料当阅读文本印在卷上，注明"听力材料"）；③用户自备音频（本技能只出题干与选项，音频由用户提供）。将选择写入 `meta.decisions.listening_plan`，Phase 2 命题严格按此执行，**不得在写到听力大题时才发现无法生成音频**。
...
> 这些参数对应 `assemble.py init` 的 `--stage --subject --region --type --total --duration --questions`，以及结构 JSON（`--blueprint-file`）。**理科须在本轮确认配图方案**：①全部改文字描述（无图版，可直接出卷）②含图题由用户提供图片（Phase 3 用 figure 块插入）③不需要含图题（调整题型规避）；将选择写入 `meta.decisions.figure_plan`。

🔴 **STOP·第三轮·补充项（极易漏，逐项确认；有默认的可直接用默认；写入 meta.json 后才进入出题）**：
...
| 英语·阅读/完形 | 初中单篇 150-250 词 / 高中单篇 250-350 词 | 记叙/说明/应用文 | VOA 分级、21 世纪英文报、China Daily |
...
**科目路由（按科目只读对应区块，跳过无关节）**：

| 科目 | 必读区块（按顺序）|
|------|-----------------|
| 语文 | ① 通用命题规则 → ② 语文专属 → ③ 选文排版铁律 |
| 英语 | ① 通用命题规则 → ③ 选文排版铁律（阅读/完形行）|
| 政治·历史·地理 | ① 通用命题规则（完成后直接进 Phase 3；详细限制见”特殊处理·政史地”节）|
| 数学·物理·化学·生物 | ① 通用命题规则 → ④ 理科补充 |

> **自洽说明**：①通用命题规则（去AI感、难度控制、能力层次）已完整内嵌在本节，**无需外部文件即可执行**。`references/exam-templates.md` 和 `references/anti-ai-guidelines.md` 是扩展补充，存在时参照，缺失时本节内嵌内容即为兜底。
...
#### ③ 选文排版铁律（语文 + 英语阅读/完形题必守，build 有硬门禁）
...
- **含图题执行**：严格按 `meta.decisions.figure_plan` 执行（三选一含义见”适用范围⚠️”）；`figure.src` 留空时 build 自动输出 `［图：…］` 占位，交付时必须列入待办清单告知用户补图。
- **无阅读选文门禁**：理科无 material 选文，溯源门禁不触发；但题干内嵌数据/公式须真实正确，实验数据需核查，禁止编造结果。
- **知识点锚定**：严格按 `meta.textbook`（Phase 1 第三轮已确认的教材版本/册次）和课标确定考查范围；解答题须给出完整步骤与公式；多选题/实验探究题在每题 `meta.type` 中声明类型。
...
**退出条件**：用户明确说"不用改了"或"定稿"后结束迭代，进入交付。
...
### 英语科目特别说明（听力·作文·字数）
- **听力**：→ 见 Phase 1 第一轮 🔴 听力处理三选一（省略/文字稿/用户自备音频），选择必须写入 `meta.decisions.listening_plan`，此处不重复。
...
> 🔴 **高中政史地无预设文件**：`presets/` 目前仅有九年级政治/历史，高中三科均无内置预设。接到高中政史地出卷请求时，**必须在 Phase 1 第一轮明确告知用户**："本卷将按通用骨架出题，建议提供一份本地样卷让我解析，否则分值/大题结构可能与你校/本省实际不符。"用户无样卷→按通用兜底，并在交付免责清单中注明。

- **时政时效（政治/道法）**：时政材料务必抓**近 1-3 个月**真实报道（月考期末）或近半年热点（学考），抓取后核对日期，过旧即换；绝不凭记忆编时政。
- **教材版本/册次**：政史地考点强绑定教材版本与册次，务必先与用户确认（见第三轮补充项）。
- **图（地图/图表/文物图）**：→ 配图限制与处理方式同理科（见"适用范围"⚠️）。政史地额外注意：历史地图/地理等高线图/文物图无法文字描述替代时，必须由用户提供原图；缺图位置输出 `［图：…］` 占位，交付时必须列入待办清单。
...
| 2 | **自行生成听力题而不告知用户** | 输出的 Word 没有音频，听力题成废题 | 开工前三选一（见英语科目说明），写入 meta.decisions |
