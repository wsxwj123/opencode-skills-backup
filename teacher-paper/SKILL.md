...
### Phase 3.5：审题磨题（Bug-P-2 修复后新增，真实教师流程的"自审"步）

**目标**：在交给用户审之前，AI 自己先按真实教师交叉审稿习惯过一遍。

🔴 **三个自审清单（必做，不可跳过）**：

1. **考点覆盖审查**：对照 manifest 列出"已考考点 vs 课标要求"，列出**漏考**与**重复考**的考点（如同一篇文章考了 3 道"修辞手法"，但"段落作用"零题）。
2. **难度梯度审查**：按板块统计各题难度系数（0.45-0.85 区间），看是否符合"基础送分→中档区分→拉分压轴"的递进，揪出"全板块难度均匀"或"前易后难突变"。
3. **风格统一性审查**：全卷题干语气（正式/亲和）、设问句式（"请简要分析"/"说说你的看法"）、解析详略——前后是否一致。AI 出卷最常见的问题是"前几道题正式后几道题随意"。

输出方式：在 build 后、给用户看之前，先**打印一张"自审表"**（板块×维度），用户能一眼看出哪里需要返工。可用工程内 `00_manifest.md` 末尾追加自审表。

> 不要把自审做成走过场——若考点漏覆盖或难度反梯度，立即返工对应 items，重跑 build；改完后再进入 Phase 4 让用户审。

🔴 **STOP·Phase 3.5 出口**：自审表全绿（无漏考、梯度合理、风格一致）后，才进入 Phase 4。
